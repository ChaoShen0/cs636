...
def _blur2d(x, f=[1,2,1], normalize=True, flip=False, stride=1):
    ...
    if flip:
        f = f[::-1, ::-1]
    f = f[:, :, np.newaxis, np.newaxis]  # 3*3*1*1  2*2*1*1
    f = np.tile(f, [1, 1, int(x.shape[1]), 1]) # 3*3*in_channel*1   2*2*in_channel*1
...
def conv2d(x, fmaps, kernel, **kwargs):
    ...
    return tf.nn.conv2d(x, filter=spectral_norm(w), strides=[1, 1, 1, 1], padding='SAME', data_format='NCHW')
...
def upscale2d_conv2d(x, fmaps, kernel, fused_scale='auto', **kwargs):
    ...
    return tf.nn.conv2d_transpose(x, filter=spectral_norm(w), output_shape = os, strides=[1,1,2,2], padding='SAME', data_format='NCHW')
...
def conv2d_downscale2d(x, fmaps, kernel, fused_scale='auto', **kwargs):
    ...
    w = tf.cast(w, x.dtype)
    return tf.nn.conv2d(x, filter=spectral_norm(w), strides=[1,1,2,2], padding='SAME', data_format='NCHW')
...
#----------------------------------------------------------------------------
# Self-attention.
def conv_at(x, channel, kernel=1, stride=1):
    return tf.layers.conv2d(inputs=x, filters=channel, kernel_size=kernel,
...
def flatten(x, res):
    ...
    if x_shape[0] == None:
        x_shape[0] = -1
    return tf.reshape(x, shape=[x_shape[0], x_shape[1], size])

def attention(x, ch, res, scope='attention', reuse=False):
    with tf.variable_scope(scope, reuse=reuse):
        # print("x_shape")
        # print(x.shape)
        f = conv_at(x, ch // 8, kernel=1, stride=1)  # [bs, c', h, w]
        g = conv_at(x, ch // 8, kernel=1, stride=1)  # [bs, c', h, w]
        h = conv_at(x, ch, kernel=1, stride=1)  # [bs, c, h, w]
        ...
        # print(h.shape)
        # N = h * w
        s = tf.matmul(flatten(g, res), flatten(f, res), transpose_a=True)  # # [bs, N, N]

        # print("flatten g")
        # print(flatten(g, res).shape)
        # print("flatten f")
        # print(flatten(f, res).shape)

        beta = tf.nn.softmax(s)  # attention map
        # print("s")
        # print(s.shape)
        # print("flatten h")
        # print(flatten(h, res).shape)
        o = tf.matmul(flatten(h, res), beta)  # [bs, N, C]
